const ObservedProperties = (SuperClass) => class extends SuperClass {
  ...
  static __propertyValueChanged(propName, oldValue, newValue) {
    ...
    try { if(JSON.stringify(oldValue) === JSON.stringify(newValue)) { return;
    }
 } catch(e) {}    this.propertyChangedCallback && this.propertyChangedCallback(propName, oldValue, newValue);
  }
...
var TinyColor = (function () {
    ...
    TinyColor.prototype.toPercentageRgbString = function () {
        ...
        return this.a === 1 ?
            "rgb(" + rnd(this.r) + "%, " + rnd(this.g) + "%, " + rnd(this.b) + "%)": "rgba(" + rnd(this.r) + "%, " + rnd(this.g) + "%, " + rnd(this.b) + "%, " + this.roundA + ")";
    };
...
class ColorPicker extends PropertiesChangedHandler(PropertiesChangedCallback(PropertyChangedHandler(Properties(HTMLElement)))) {
  ...
  _handleGridKeydown(e) {
    ...
    if(e.key.indexOf()) {
      if(e.key === 'ArrowLeft') return this.value = (this.selectedFormat === 'hsl') ? {...hsl, ...{s: hsl.s-0.01}}
    } : {...hsv, ...{s: hsv.s-0.01}};
