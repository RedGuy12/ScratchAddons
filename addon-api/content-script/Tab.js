...
export default class Tab extends Listenable {
	constructor(info) {
		...
		this.clientVersion = document.querySelector(
	 ...
	 */
	copyImage(dataURL) {
		if (!dataURL.startsWith(DATA_PNG)) {
			return Promise.reject(new TypeError("Expected PNG data URL"));
		}
		...
		if (typeof Clipboard.prototype.write === "function") {
			...
			return scratchAddons.methods.copyImage(dataURL).catch((err) => {
				return Promise.reject(
					new Error(`Error inside clipboard handler: ${err}`),
				);
			});
		}
	}
	...
	scratchMessage(key) {
		if (this.clientVersion === "scratch-www") {
			const locales = [
				window._locale ? window._locale.toLowerCase() : "en",
			];
			if (locales[0].includes("-")) {
				locales.push(locales[0].split("-")[0]);
			}

			if (locales.includes("pt") && !locales.includes("pt-br")) {
				locales.push("pt-br");
			}
	...
	scratchClass(...args) {
		let res = "";
		args.filter((arg) => typeof arg === "string").forEach(
			(classNameToFind) => {
